...
export default function ProjectForm({ initialData }: ProjectFormProps) {
  ...
  return (
    <form onSubmit={handleSave} className="space-y-8">
      {/* Header */}
      <div className="flex items-center justify-between bg-bg-card p-4 rounded-xl border border-border sticky top-4 z-40">
        <div className="flex items-center gap-4">
          <Link href="/admin/projects" className="p-2 text-text-secondary hover:text-text-primary hover:bg-bg-elevated rounded-lg transition-colors">
        ...
        <div className="flex items-center gap-4">
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              className="w-4 h-4 rounded border-border-subtle text-[#3B82F6] focus:ring-[#3B82F6] bg-bg-elevated"
            />
          ...
          <button
            ...
            className="flex items-center gap-2 bg-[#3B82F6] hover:bg-[#2563EB] disabled:opacity-50 text-text-primary px-6 py-2 rounded-lg font-medium transition-colors"
          >
            {isSaving || uploading ? (
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="xl:col-span-2 space-y-6">
          <div className="bg-bg-card border border-border rounded-xl p-6 space-y-6">
            <h2 className="text-lg font-semibold text-text-primary mb-4 border-b border-border pb-2">Temel Bilgiler</h2>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-text-secondary mb-1">Kısa Açıklama *</label>
                <textarea
                  required
                  rows={2}
              ...
              <div data-color-mode="dark">
                <label className="block text-sm font-medium text-text-secondary mb-1">Detaylı Açıklama (Markdown)</label>
                <MDEditor
          ...
          <div className="bg-bg-card border border-border rounded-xl p-6 space-y-6">
            <h2 className="text-lg font-semibold text-text-primary mb-4 border-b border-border pb-2">Medya ve Dosyalar</h2>

            {/* Cover Image */}
            <div>
              <label className="block text-sm font-medium text-text-secondary mb-2">Kapak Görseli</label>
              ...
              <div className="mb-3">
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-bg-elevated border border-border-subtle rounded-lg px-4 py-2 text-text-primary text-sm focus:outline-none focus:border-[#3B82F6] placeholder-text-muted"
                  />
                  {formData.coverImage && (
                ...
                {formData.coverImage && !coverFile && (
                  <div className="mt-2 rounded-lg overflow-hidden border border-border-subtle aspect-video max-h-40">
              ...
              <p className="text-xs text-text-muted mb-2">veya dosya yükle</p>
              <div 
                {...getCoverProps()} 
                className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-colors ${
                  isCoverDrag ? 'border-[#3B82F6] bg-[#3B82F6]/10' : 'border-border-subtle hover:border-[#444444] bg-bg-elevated'
                }`}
              >
            ...
            {/* Gallery */}
            <div>
              <label className="block text-sm font-medium text-text-secondary mb-2">Galeri Görselleri</label>
              ...
              <div className="flex gap-2 mb-3">
                ...
                <button
                  ...
                  className="px-3 py-2 bg-[#3B82F6] hover:bg-[#2563EB] text-text-primary rounded-lg transition-colors"
                >
              ...
              <div 
                ...
                className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-colors mb-4 ${
                  isGalleryDrag ? 'border-[#3B82F6] bg-[#3B82F6]/10' : 'border-border-subtle hover:border-[#444444] bg-bg-elevated'
                }`}
              >
              ...
              {(formData.galleryImages.length > 0 || galleryFiles.length > 0) && (
                <div className="grid grid-cols-4 sm:grid-cols-5 gap-3">
                  {formData.galleryImages.map((url, i) => (
                    <div key={url} className="relative aspect-video rounded-lg overflow-hidden border border-border-subtle group">
                      <img src={url} alt="" className="w-full h-full object-cover" />
                      <button type="button" onClick={() => removeGalleryImage(i)} className="absolute top-1 right-1 p-1 bg-red-500 rounded text-text-primary opacity-0 group-hover:opacity-100 transition-opacity">
                        <X className="w-3 h-3" />
                      </button>
                    </div>
                  ))}
                  {galleryFiles.map((file, i) => (
                    <div key={i} className="relative aspect-video rounded-lg overflow-hidden border-2 border-dashed border-[#3B82F6] opacity-70 group">
                      ...
                      <button type="button" onClick={() => removeGalleryFile(i)} className="absolute top-1 right-1 p-1 bg-red-500 rounded text-text-primary">
                        <X className="w-3 h-3" />
                      </button>
                    </div>
                  ))}
            ...
            {/* ZIP File */}
            <div>
              <label className="block text-sm font-medium text-text-secondary mb-2">İndirilebilir Dosya</label>
              ...
              {formData.zipFile && (
                <div className="mb-3">
                  <label className="block text-xs text-text-muted mb-1">Dosya adı (gösterilecek)</label>
                  <input
                    type="text"
                    value={formData.zipFileName}
                    onChange={(e) => setFormData(prev => ({ ...prev, zipFileName: e.target.value }))}
                    placeholder="proje.zip"
                    className="w-full bg-bg-elevated border border-border-subtle rounded-lg px-4 py-2 text-text-primary text-sm focus:outline-none focus:border-[#3B82F6]"
                  />
                </div>
              )}
              ...
              <div
                {...getZipProps()}
                className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-colors ${
                  isZipDrag ? 'border-green-500 bg-green-500/10' : 'border-border-subtle hover:border-[#444444] bg-bg-elevated'
                }`}
              >
                <input {...getZipInputProps()} />
                <div className="flex flex-col items-center gap-2">
                  <FileArchive className="w-6 h-6 text-text-secondary" />
                  {zipFileObj ? (
                    <p className="text-green-500 font-medium text-sm">Seçildi: {zipFileObj.name} ({formatFileSize(zipFileObj.size)})</p>
                  ) : (
                    <p className="text-text-secondary text-sm">ZIP dosyası sürükleyin veya tıklayın</p>
                  )}
                </div>
              </div>
            </div>
        ...
        <div className="space-y-6">
          <div className="bg-bg-card border border-border rounded-xl p-6 space-y-4">
            <h2 className="text-lg font-semibold text-text-primary border-b border-border pb-2">Meta & Sınıflandırma</h2>
            ...
            <div>
              <label className="block text-sm font-medium text-text-secondary mb-1">Sürüm</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-medium text-text-secondary mb-1">Çıkış Tarihi</label>
              <input
                type="date"
            ...
            <div>
              <label className="flex items-center gap-2 cursor-pointer mt-4 p-3 bg-bg-elevated border border-border-subtle rounded-lg">
                <input
                  type="checkbox"
                  checked={formData.featured}
                  onChange={(e) => setFormData(prev => ({ ...prev, featured: e.target.checked }))}
                  className="w-4 h-4 rounded border-border-subtle text-purple-500 focus:ring-purple-500 bg-bg-card"
                />
                <div>
                  <span className="text-sm font-medium text-text-primary block">Öne Çıkan Proje</span>
                  <span className="text-xs text-text-secondary">Ana sayfada sergile</span>
                </div>
              </label>
            </div>
          </div>
          ...
          <div className="bg-bg-card border border-border rounded-xl p-6 space-y-4">
            <h2 className="text-lg font-semibold text-text-primary border-b border-border pb-2">Bağlantılar & Etiketler</h2>
            ...
            <div>
              <label className="block text-sm font-medium text-text-secondary mb-1">Canlı Demo URL</label>
              <input
                type="url"
            ...
            <div>
              <label className="block text-sm font-medium text-text-secondary mb-1">Etiketler (Virgülle ayırın)</label>
              <input
                type="text"
                value={formData.tags}
            ...
            <div>
              <label className="block text-sm font-medium text-text-secondary mb-2">Teknolojiler</label>
              <div className="flex flex-wrap gap-2 max-h-48 overflow-y-auto p-2 bg-bg-elevated border border-border-subtle rounded-lg">
                  ...
                  <button
                    ...
                    className={`px-2 py-1 text-xs rounded border transition-colors ${
                      formData.technologies.includes(tech)
                        ...
                        : 'bg-bg-card border-border-subtle text-text-secondary hover:border-[#555]'
                    }`}
                  >
                    {tech}
                  </button>
